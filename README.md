...
NursePath is currently in **demo / faculty-review** with **Universidad de Dagupan's College of Nursing**. It is not yet formally adopted. Faculty and selected students may use it for classroom demos, sim lab practice, and evaluation ahead of any official rollout.
...
While in demo, NursePath is intended for classroom demos, sim lab practice, and supervised evaluation — not as an officially adopted ward tool. Treat outputs the same way you would a pocket drug handbook or vital signs reference card: helpful context, never a substitute for your Clinical Instructor or hospital protocols.
...
| Faculty Review | 🔄 In Progress | Demo shared with College of Nursing faculty |
| **University Adoption** | ⏳ **Not yet** | Still in demo — not formally adopted |
| Student Feedback | 🔄 In Progress | Gathering feedback from demo users |
| Formal Rollout Planning | ⏳ Pending | Depends on faculty evaluation outcomes |
...
Contact the developer or faculty lead for dashboard credentials. Do not share access credentials.
...
- The open Supabase insert policy (`with check (true)`) is acceptable for a supervised demo — tighten it before any public production deployment
